...
def two_pass_labeling(B):
    ...
    order = set_order(labels)
    labels = imaging(labels, order)
    return labels


def set_order(labels):   
    numbers = {j:k for j, k in zip(np.unique(labels), range(len(labels)))}
    return numbers


def imaging(image, order):
    image = image.copy()
